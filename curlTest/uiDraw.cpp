...
*   INPUT  location  The location of the selected square
************************************************************************/
void ogstream::drawPossible(Move move)
{
   // do nothing if there is nothing to do
   if (move.getDestination().getLocation() < 0 || move.getDestination().getLocation() >= 64)
      return;

   // find the row and column
   int row = move.getDestination().getLocation() / 8;
   int col = move.getDestination().getLocation() % 8;

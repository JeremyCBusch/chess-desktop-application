...
tuple<int, string, gameSelectorReturnType> gameSelector::menuLoop()
{
	string userInput;
	...
	tuple <int, string, gameSelectorReturnType> returnData;
	...
	while (*stillInLoop == true)
	{
		...
		case(2):
			...
			gameID = stoi(createGame(userName));
			stillInLoop = new bool(false);

			returnData = make_tuple(gameID, userName, gameSelectorReturnType(CREATE));
		...
		case(3):
			...
			returnData = make_tuple(gameID, userName, gameSelectorReturnType(JOIN));
...
string gameSelector::createGame(string userName)
{
	vector<string> params = vector<string>();
